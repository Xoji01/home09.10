...
true
    ...
    for(item of arr) 
{let filte=arr.filter((item)=> item.completed===true)
    a.count=a.count +filte.length
a.arr.push(filte)
let filt=arr.filter((item)=> item.completed===false)
    b.count=b.count +filt.length
a.arr.push(filt)} 
...
for(let i=0;i<users.length;i++) {
    let item=users[i]
    let eaa=item.email.split('.').at(-1)
    if (emails[eaa]){emails[eaa].push(item.email)
    }else {
